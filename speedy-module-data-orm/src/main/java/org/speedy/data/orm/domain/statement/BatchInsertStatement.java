...
public class BatchInsertStatement extends BatchStatement {
    ...
    public BatchInsertStatement values() {
        ...
        // 找出需要执行插入操作的字段列表
        List<Field> insertFieldList = Arrays.stream(targetFields)
                .filter(f -> !IGNORE_FIELD.contains(f.getName()))
                .filter(f -> ReflectUtils.methodGetFieldValue(insertObject, f) != null)
    ...
    SqlStatement complete() {
        ...
        // 拼接占位符
        String placeholderString = placeholderList.stream()
                .collect(Collectors.joining(delimiter, prefix, suffix));
        ...
        // 拼接完整的sql语句
        this.sql = stringBuilder.append("insert into ").append(target)
                .append(insertFieldNameString).append(" values ")
